...
estructura_db = leer_estructura_sql("D:\\CCOMP\\CCOMP - UCSP\\Quinto Semestre\\BD2\\proyecto\\code\\5ProyectoFinal\\struct_tableNew.txt")
tam_registro  = sum(c['tam'] for c in estructura_db)

print(f"\ntam_registro : {tam_registro} bytes")

registros = leer_csv("D:\\CCOMP\\CCOMP - UCSP\\Quinto Semestre\\BD2\\proyecto\\code\\5ProyectoFinal\\Inventory_v4.csv")
    ...
    if modo == '1':
        ...
        if resultado is None:
            ...
            for idx, (n_reg, plato) in enumerate(resultado, 1):
                ...
                p, sup, pi, sec, _ = disco._offset_a_dir(offset)
                print(f"  [{idx}] n_reg={n_reg} | offset={offset} | Plato:{p} | Sup:{sup} | Pista:{pi} | Sector:{sec}")
